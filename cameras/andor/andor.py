...
with open(os.path.join('config', 'logging.json')) as data_file:
    params = json.load(data_file)
...
class Controller:
    def __init__(self, cam_prefix="ifu", serial_number="test",
                 ...
                 remote_config='test.config.json'):
        """
    ...
    def initialize(self, path_to_lib="", wait_to_cool=True):
        ...
        try:
            ...
            self.opt.SetVSSpeed(self.VerticalShiftSpeed_States[self.VerticalShiftSpeed])
            self.opt.SetReadMode(self.ReadModes[self.ReadMode])
            self.opt.SetAcquisitionMode(self.AcquisitionModes[self.AcquisitionMode])
    ...
    def take_image(self, shutter='normal', exptime=0.0,
        ...
        # 2. Convert exposure time to milliseconds (ms)  #Andor exposure times are in seconds
        try:
        ...
        try:
            # imdata = self.opt.readNFrames(N=1, timeout=timeout)[0][0]  # Don't know what this is either
            self.opt.StartAcquisition()
            imdata = []
            self.opt.GetAcquiredData16(imdata, width=self.ROI[3], height=self.ROI[5])
        except Exception as e:
            self.lastError = str(e)
        ...
        try:
            ...
            hdul.header.set("VSSPEED", self.VerticalShiftSpeed, "VS Speed in um / pixel shift")
