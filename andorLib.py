...
class Andor:
    ...
    def GetDetector(self):
        ...
        check_call(self.lib.GetDetector(byref(detector_width), byref(detector_height)))
    ...
    def GetFastestRecommendedVSSpeed(self):
        ...
        check_call(self.lib.GetFastestRecommendedVSSpeed(byref(fastest_vss_index), byref(fastest_vss_speed)))
    ...
    def GetHSSpeed(self, adc_channel, output_amp, hss_index):
        hs_speed = c_float()
        check_call(
            self.lib.GetHSSpeed(c_int(adc_channel), c_int(output_amp), c_int(hss_index), byref(hs_speed)))
    ...
    def GetNumberHSSpeeds(self, channel, typ):
        speeds = c_int()
        check_call(self.lib.GetNumberHSSpeeds(c_int(channel), c_int(typ), byref(speeds)))
    ...
    def saveFits(self):
        ...
        hdul.header.set("HSSPEED", self.GetHSSpeed(0, 0, self.hs_index), "HS speed in MHz")
    ...
    def SetImage(self, hbin, vbin, hstart, hend, vstart, vend):
        status = check_call(self.lib.SetImage(c_int(hbin), c_int(vbin), c_int(hstart), c_int(hend), c_int(vstart),
                                              c_int(vend)))
        return ERROR_STRING[status]
